...
def test_trace_is_nonincreasing_after_sweep_fix():
    ...
    Y = np.column_stack([Xs[j]@B_true[j] for j in range(K)]) + Z@F_true.T + rng.standard_normal((N,K))*np.sqrt(D_true)

    _,_,_,_,info = als_gls(Xs,Y,k=3,sweeps=15,cg_maxit=4000,cg_tol=1e-8,scale_correct=True)
