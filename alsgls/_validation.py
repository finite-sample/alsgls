...
def _check_array_compatibility(
    Xs: list[np.ndarray], Y: np.ndarray, *, X_name: str = "Xs", Y_name: str = "Y"
) -> None:
...
def _sanitize_regularization_params(
    lam_F: float | None = None, lam_B: float | None = None
) -> tuple[float, float]:
...
def _validate_convergence_params(
    sweeps: Any = None, rel_tol: Any = None, cg_maxit: Any = None, cg_tol: Any = None
) -> dict[str, int | float]:
